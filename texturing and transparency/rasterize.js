...
var textureBuffers = [];
var is_lit = 2; //init using blinn-phong shading
...
function check_mipmap (tri_texture,mipmap) {
	if (mipmap) {
        set_mipmap();
     } else {
        set_wrapping();
...
function handle_texture_load (tri_texture,img_width,img_height) {
	var mipmap = false;
    ...
    check_mipmap(tri_texture,mipmap);
    gl.bindTexture(gl.TEXTURE_2D, null);
}

function setup_texture(img_name) {
    var tri_texture = gl.createTexture();
    tri_texture.image = new Image();
    tri_texture.image.onload = function() {
    	img_width = tri_texture.image.width;
    	img_height = tri_texture.image.height;
        handle_texture_load(tri_texture,img_width,img_height);
    };
    tri_texture.image.crossOrigin = "Anonymous";
    var path = "https://ncsucgclass.github.io/prog4/" + img_name;
    tri_texture.image.src = path;
    return tri_texture;
}
    ...
    try {
        if (inputTriangles == String.null)
            ...
            for (var whichSet=0; whichSet<numTriangleSets; whichSet++) { // for each tri set
                ...
                if (img_name != false){
                	inputTriangles[whichSet].texture_obj = setup_texture(img_name);
                }
                else{
                	inputTriangles[whichSet].texture_obj = false;
                }
                inputTriangles[whichSet].tx_lightmap_obj = setup_texture('lightmap.gif');
...
function setupShaders() {
    ...
    var vShaderCode = `
        ...
        varying vec2 vVertexTexture; // texture for frag shader
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                vTexAttribLoc = gl.getAttribLocation(shaderProgram, "aVertexTexture"); // ptr to vertex tex attrib
                gl.enableVertexAttribArray(vTexAttribLoc); // connect attrib to array
                sampler_unlit = gl.getUniformLocation(shaderProgram, "unlitSampler");
                sampler_lit = gl.getUniformLocation(shaderProgram, "litSampler");
                lit_shader = gl.getUniformLocation(shaderProgram, "texture_lit");
                u_alpha = gl.getUniformLocation(shaderProgram, "alpha_val");
                is_texture = gl.getUniformLocation(shaderProgram, "is_tex");
...
function compute_depth (tri_set) {
 	tri_center = inputTriangles[tri_set].center[2];
 	trans = inputTriangles[tri_set].translation[2];
 	return tri_center+trans;
}
...
function renderModels() {
    ...
    function draw_tri (whichTriSet) {
        ...
        gl.uniform1i(lit_shader, is_lit);
        ...
        if (inputTriangles[whichTriSet].texture_obj != false){
        	...
        	gl.uniform1i(sampler_unlit, 0);
        	gl.uniform1i(is_texture,1);
        }
        else{
        	gl.uniform1i(is_texture,0);
        }
        gl.activeTexture(gl.TEXTURE1);
        gl.bindTexture(gl.TEXTURE_2D, inputTriangles[whichTriSet].tx_lightmap_obj);
        gl.uniform1i(sampler_lit, 1);
    ...
    for(let i = 0, length1 = transparent_triangles.length; i < length1; i++){
    	z_val = compute_depth(transparent_triangles[i]);
...
// https://web.archive.org/web/20171109162523/http://learningwebgl.com/blog/?p=571
// https://web.archive.org/web/20180317215242/http://learningwebgl.com/blog/?p=507
